        ...
        for student in students:
            ...
            try:
                ...
                if _check_if_learner_on_blocklist(course_key, user):
                    ...
                    log.warning(f'Student {user.username} is blocked from receiving a Certificate in Course '
                                f'{course_key}')
                # make sure user isn't already on the exception list
                    ...
                    log.warning(f'Student {user.username} already on exception list in Course {course_key}.')
                # make sure user is enrolled in course
                elif not CourseEnrollment.is_enrolled(user, course_key):
                    build_row_errors('user_not_enrolled', user, row_num)
                    log.warning(f'Student {user.username} is not enrolled in Course {course_key}')
                else:
...
def _get_certificate_for_user(course_key, student):
    ...
    """
    log.info(f"Retrieving certificate for {student} in course {course_key}")
...
def _check_if_learner_on_blocklist(course_key, student):
    ...
    """
    log.info(f"Checking if {student} is currently on the blocklist of course {course_key}")
